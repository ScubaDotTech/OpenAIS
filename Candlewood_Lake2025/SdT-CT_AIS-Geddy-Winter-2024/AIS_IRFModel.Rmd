...
set.seed(123)
x <- model.matrix(point_sum_cerdem ~ ., data = eco_data)[, -1]
y <- eco_data$point_sum_cerdem

lambda_grid <- 10^seq(10, -2, length = 2000)

# Initialize a vector to store the Mean Squared Errors (MSE)
mse_values <- numeric(length(lambda_grid))
...
####################################################################################
######################## For Loop (determine direction of refitting) ############################ 
####################################################################################
# Prepare data
x <- model.matrix(FutureClosePrice_Outcome ~ ., data = impp_dataDT)[, -1]  # Remove intercept column
y <- impp_dataDT$FutureClosePrice_Outcome

# Grid of lambda values for tuning
lambda_grid <- 10^seq(10, -2, length = 2000)
...
# Set up for the for loop to iterate over the lambda values
for (i in 1:length(lambda_grid)) {
  # Fit the Lasso model for the current lambda
  lasso_mod <- cv.glmnet(x, y, alpha = 1, lambda = lambda_grid, nfolds = 10, thresh = lambda_grid[i],)
  
  # Make predictions on the test set (assuming you have a train-test split)
  y_pred <- predict(lasso_mod, s = lambda_grid[i], newx = x[test,])
  
  # Compute Mean Squared Error (MSE) for the current model
  mse_values[i] <- mean((y_pred - y[test])^2)
}
...
twenty <- twenty %>% dplyr::select(year, transect, point, lattitude, longitude, year, depth_m_, dfs_m_, substrate.code, june_temp, june_dew_point, point_sum_myrspi, point_sum_cerdem, point_sum_others)


twenty$year <- 2021 #overide so that year is involved in the calculations
twenty$june_temp <- 73.29
twenty$june_dew_point <-  64.08 


twenty <- na.omit(twenty)


resp_1 <- "point_sum_myrspi"

colnames(twenty)
# Generate predictions for the entire datasetwenty# Generate predictions for the entire dataset at once
pre <- predict(rf_model, newdata = twenty)  
predict_cerdem <- predict(rf_model_cerdem, newdata = twenty)  
predict_others <- predict(rf_model_others, newdata = twenty)

# Create a new column name and assign the predictions
pred_colname <- paste0("pred_", resp_1)
twenty[[pred_colname]] <- round(pre)

twenty_one <- twenty %>%
  dplyr:: select(-point_sum_myrspi, -point_sum_cerdem, -point_sum_others) %>%
  mutate(point_sum_myrspi = pred_point_sum_myrspi) %>% dplyr :: select(-pred_point_sum_myrspi)
