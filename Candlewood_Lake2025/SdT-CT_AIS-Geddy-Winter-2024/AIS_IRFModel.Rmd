Geddy Lucier
Testing out a Itterative Random Forest and Lasso Model for Candlewood Lake Future Ecological Data 
3/11/2024

Predicting Myrspi, Cerdem, and Other species total. 
...
summary(linear) # significance testing 
...
Other iterative lasso performs worse than cerdem (coontail) model. Likely attributed due to the aggregated and "other" nature of the different species that make up the category. 
...
twenty_one$june_temp <- 73.29 # temperature 
twenty_one$june_dew_point <-  64.08  # dewpoint
...
twenty_one$point_sum_cerdem <-round(predict_cerdem)
 ...
 # generating others using the random forest 
...
eco_final <- as.data.frame(eco_final)
eco_final$point_sum_cerdem <- as.vector(eco_final$point_sum_myrspi) # transforming from list to vector
eco_final$point_sum_cerdem <- as.vector(eco_final$point_sum_cerdem)
eco_final$point_sum_others <- as.vector(eco_final$point_sum_others)
...
write_csv(eco_final, file = "CL_ecodata_final.csv")
